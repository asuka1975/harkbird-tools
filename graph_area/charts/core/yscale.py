...
void main() {
    ...
    trans_tick = trans_tick * (2.0 - padding.y * 2.0) / 2.0;

    if(gl_VertexID % 2 == 0) {
        gl_Position = vec4(-1 + padding.x + 0.008, trans_tick, 0, 1);
    } else {
        gl_Position = vec4(-1 + padding.x - padding.x * 0.1, trans_tick, 0, 1);
    }
}
...
class YScale:
    def __init__(self, padding, range_, tick, color=(0, 0, 0, 1)):
        self.program = Shader()
